...
async function deployFortuneSeekerContract(signer: Signer, keyStore: KeyStore, adminAddress: string, oracleAddress: string, tokenAddress: string): Promise<string> {
    ...
    const storage =`{
        "prim": "Pair",
        "args": [
          { "prim": "Pair", "args": [ { "string": "${adminAddress}" }, { "prim": "Pair", "args": [ { "int": "0" }, { "bytes": "0001" } ] } ] },
          {
            "prim": "Pair",
            "args": [ { "prim": "Pair", "args": [ { "int": "1" }, { "string": "${oracleAddress}"} ] }, { "prim": "Pair", "args": [ { "string": "${tokenAddress}" }, { "prim": "None" } ] } ]
          }
        ]
      }`;
    */
